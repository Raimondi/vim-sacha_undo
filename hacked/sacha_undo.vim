...
function! Ascii(buf, state, type, char, text, coldata)
  ...
  let add_padding_line = len(a:text) > 2 && coldiff == -1
        \ && !empty(filter(deepcopy(edges), 'v:val[0] + 1 < v:val[1]'))
...
function! Make_nodes(alts, nodes, parent) "{{{1
  " let parent = a:0 ? a:1 : {}
  let parent = a:parent
  for alt in a:alts
...
function! sacha_undo#make_nodes() "{{{1
  ...
  call Make_nodes(entries, nodes, nodes[0])
  let nmap = {}
  for node in nodes
...
function! sacha_undo#render_graph()
  ...
  call PrintNodes(nodes)
  let dag = sort(nodes, 's:compare_fn')
  let current = Changenr(nodes)

  let string = Generate(
        \ map(dag, '[v:val, !empty(v:val.parent) ? [v:val.parent] : []]'),
        \ 'Asciiedges', current
        \ )
